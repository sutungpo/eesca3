...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        p2d = (pad, pad, pad, pad)
        x_padded = torch.nn.functional.pad(x, p2d)
        out = torch.zeros((N, F, H_p, W_p), dtype=x.dtype, device=x.device)
        for i in range(H_p):
            for j in range(W_p):
                x_slice = x_padded[
                ...
                out[:, :, i, j] = x_slice.mm(w_reshaped.t()) + b
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        # Padding
        x_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad)).to(x)
        N, C, Hpad, Wpad = x_pad.shape
        N, F, outH, outW = dout.shape
        _, _, FH, FW = w.shape

        dx = torch.zeros_like(x).to(x)
        dw = torch.zeros_like(w).to(x)
        db = torch.zeros_like(b).to(x)
        # Naive Loops
        ...
        x_col = torch.zeros(C * FH * FW, outH * outW).to(x)  # [C*FH*FW x H'*W']
        for index in range(N):
            out_col = dout[index].reshape(F, outH * outW)  # [F x H'*W']
            w_out = w_row.T.mm(out_col)  # [C*FH*FW x H'*W']
            dx_cur = torch.zeros(C, Hpad, Wpad).to(x)
            neuron = 0
            for i in range(0, Hpad - FH + 1, stride):
                for j in range(0, Wpad - FW + 1, stride):
                    dx_cur[:, i : i + FH, j : j + FW] += w_out[:, neuron].reshape(
                        C, FH, FW
                    )
                    x_col[:, neuron] = x_pad[index, :, i : i + FH, j : j + FW].reshape(
                        C * FH * FW
                    )
                    neuron += 1
            dx[index] = dx_cur[:, pad:-pad, pad:-pad]
            dw += out_col.mm(x_col.T).reshape(F, C, FH, FW)
            db += out_col.sum(dim=1)
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
        return dx, dw, db
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        HH = 1 + (H - pool_h) // stride
        WW = 1 + (W - pool_w) // stride
        out = torch.zeros(N, C, HH, WW).to(x)
        ...
        for n in range(N):
            for i in range(HH):
                for j in range(WW):
                    ...
                    local_dw[range(C), indicies] = dout[n, :, i, j]
                    dx[
                        n,
                        :,
                        i * stride : i * stride + pool_h,
                        j * stride : j * stride + pool_w,
                    ] = local_dw.reshape(shape_local_x)
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params["W2"] = weight_scale * torch.randn(
            (num_filters * H * W // 4, hidden_dim), dtype=dtype, device=device
        )
    ...
    def loss(self, X, y=None):
        ...
        out2, cache2 = Linear_ReLU.forward(out1, self.params["W2"], self.params["b2"])
        scores, cache3 = Linear.forward(out2, self.params["W3"], self.params["b3"])
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dx, dw, db = Linear.backward(dx, cache3)
        grads["W3"] = dw + 2 * self.reg * (self.params["W3"])
        grads["b3"] = db + 2 * self.reg * (self.params["b3"])
        dx, dw, db = Linear_ReLU.backward(dx, cache2)
        grads["W2"] = dw + 2 * self.reg * (self.params["W2"])
        grads["b2"] = db + 2 * self.reg * (self.params["b2"])
        dx, dw, db = Conv_ReLU_Pool.backward(dx, cache1)
        grads["W1"] = dw + 2 * self.reg * (self.params["W1"])
        grads["b1"] = db + 2 * self.reg * (self.params["b1"])
        ###################################################################
        #                             END OF YOUR CODE                    #
        ###################################################################
...
class DeepConvNet(object):
    ...
    def __init__(
        ...
        #####################################################################
        # Replace "pass" statement with your code
        def weight_scale_initializer(
            Din, Dout, K=None, device="cpu", dtype=torch.float32
        ):
            if K:
                out_shape = (Dout, Din, K, K)
            else:
                out_shape = (Din, Dout)
            return torch.randn(out_shape, device=device, dtype=dtype)

        if weight_scale == "kaiming":
            initializer = kaiming_initializer
            weight_scale = 1.0
        else:
            initializer = weight_scale_initializer

        max_pool_b = [1] * len(num_filters)
        for i, _ in enumerate(max_pools):
            max_pool_b[i] = 2

        # initialize conv layers weights and bias
        Din_c, Din_h, _ = input_dims
        for i in range(1, len(num_filters) + 1):
            Dout_h = Din_h // max_pool_b[i - 1]
            Dout_c = num_filters[i - 1]
            self.params[f"W{i}"] = weight_scale * initializer(
                Din_c, Dout_c, 3, device=device, dtype=dtype
            )
            self.params[f"b{i}"] = torch.zeros(Dout_c, dtype=dtype, device=device)
            Din_c, Din_h = Dout_c, Dout_h
            if batchnorm:
                self.params[f"gamma{i}"] = torch.ones(
                    Dout_c, dtype=dtype, device=device
                )
                self.params[f"beta{i}"] = torch.zeros(
                    Dout_c, dtype=dtype, device=device
                )

        # initialize linear weight and bias
        self.params[f"W{self.num_layers}"] = weight_scale * initializer(
            Din_c * Din_h * Din_h, num_classes, dtype=dtype, device=device
        )
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            max_bool = i - 1 in self.max_pools
            if self.batchnorm and max_bool:
                out, cache1 = Conv_BatchNorm_ReLU_Pool.forward(
                    out,
                    self.params[f"W{i}"],
                    self.params[f"b{i}"],
                ...
                out, cache1 = Conv_BatchNorm_ReLU.forward(
                    out,
                    self.params[f"W{i}"],
                    self.params[f"b{i}"],
                    self.params[f"gamma{i}"],
                ...
                out, cache1 = Conv_ReLU_Pool.forward(
                    out,
                    self.params[f"W{i}"],
                    self.params[f"b{i}"],
                    conv_param,
                ...
                out, cache1 = Conv_ReLU.forward(
                    out, self.params[f"W{i}"], self.params[f"b{i}"], conv_param
                )
            caches.append(cache1)

        scores, cache2 = Linear.forward(
        ...
        for i in range(1, self.num_layers + 1):
            loss_reg += torch.sum(self.params[f"W{i}"] ** 2)
        loss += self.reg * loss_reg

        dx, dw, db = Linear.backward(dx, caches.pop())
        grads[f"W{self.num_layers}"] = (
            dw + 2 * self.reg * self.params[f"W{self.num_layers}"]
        )
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.batchnorm and max_bool:
                ...
                dx, dw, db = Conv_ReLU_Pool.backward(dx, caches.pop())
            else:
                dx, dw, db = Conv_ReLU.backward(dx, caches.pop())
...
def kaiming_initializer(
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = math.sqrt(gain / Din) * torch.randn(
            Din, Dout, device=device, dtype=dtype
        )
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = math.sqrt(gain / (Din * K * K)) * torch.randn(
            Dout, Din, K, K, device=device, dtype=dtype
        )
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == "train":
            ...
            # Replace "pass" statement with your code
            # mu, sigma
            mu = torch.mean(x, dim=0, dtype=x.dtype)
            xmu = x - mu
            var = ((xmu) ** 2).mean(dim=0)
            sqrtvart = torch.sqrt(var + eps)
            ivar = 1.0 / sqrtvart
            x_hat = (x - mu) * ivar

            running_mean = (1 - momentum) * mu + momentum * running_mean
            running_var = (1 - momentum) * var + momentum * running_var
            bn_param["running_mean"] = running_mean
            bn_param["running_var"] = running_var
            cache = x_hat, gamma, xmu, ivar, sqrtvart, var, eps
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == "test":
    ...
    def backward(dout, cache):
        ...
        # step5
        dvar = 0.5 * 1.0 / torch.sqrt(var + eps) * dsqrtvar

        # step4
        dsq = 1.0 / N * torch.ones((N, D)).to(dout) * dvar
        ...
        # step1
        dx2 = 1.0 / N * torch.ones((N, D)).to(dout) * dmu
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        x_flatten = torch.permute(x, (0, 2, 3, 1)).flatten(end_dim=-2)
        out_flatten, cache = BatchNorm.forward(x_flatten, gamma, beta, bn_param)
        out = out_flatten.reshape(N, H, W, C).permute(0, 3, 1, 2)
        ################################################################
        #                       END OF YOUR CODE                       #
        ################################################################
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        dout_flatten = torch.permute(dout, (0, 2, 3, 1)).flatten(end_dim=-2)
        dx, dgamma, dbeta = BatchNorm.backward(dout_flatten, cache)
