...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N = x.shape[0]
        dx = torch.mm(dout, w.t()).view(N,*x.shape[1:])
        dw = torch.mm(x.flatten(start_dim=1).t(), dout)
        db = dout.sum(dim=0)
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        grad2, grads["W2"], grads["b2"] = Linear.backward(dx, cache2)
        grads["W2"] += 2*self.reg * self.params["W2"]
        _, grads["W1"], grads["b1"] = Linear_ReLU.backward(grad2, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1,self.num_layers+1):
            self.params[f'W{i}'] = weight_scale* torch.randn((layer_dims[i-1],layer_dims[i]),dtype=dtype,device=device)
            self.params[f'b{i}'] = torch.zeros((layer_dims[i],),dtype=dtype,device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
            out, cache = Linear_ReLU.forward(out,self.params[f'W{i}'], self.params[f'b{i}'])
            caches.append(cache)
            if self.use_dropout:
        ...
        loss,dx = softmax_loss(scores,y)
        for i in range(1, self.num_layers+1):
            reg_loss += torch.sum(self.params[f'W{i}']**2).item()
        ...
        for i in range(self.num_layers-1,0,-1):
            ...
            dx, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dx,caches.pop())
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model,data_dict,optim_config={'learning_rate':1e-1,},lr_decay=0.95, num_epochs=20,batch_size = 100, print_every=1000,print_acc_every=10,device= device)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - config["learning_rate"] * dw / (
        config["cache"].sqrt() + config["epsilon"]
    )
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    moment_unbias_m = config["m"] / (1-config['beta1'] ** config["t"])
    moment_unbias_v = config["v"] / (1-config['beta2'] ** config["t"])
    next_w = w - config["learning_rate"] * moment_unbias_m / (moment_unbias_v.sqrt() + config["epsilon"])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(x.shape) > p).to(x)
            out = x * mask / (1-p)
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
